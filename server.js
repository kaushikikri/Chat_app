...
io.on('connection', socket => {
    ...
    socket.on('join-room', ({ username, chatroom }) => {
        ...
        socket.emit('message', formatmsg(`${user.username}`, 'Welcome'));

        //event broadcasting 
        socket.broadcast.to(user.chatroom).emit('message', formatmsg(`${user.username}`, `${user.username} has joined`));
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.chatroom).emit('message', formatmsg(`${user.username}`, `${user.username} has left`));
            io.to(user.chatroom).emit('roomuser', { chatroom: user.chatroom, user: getroom(user.chatroom) });
        }
    })
